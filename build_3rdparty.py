...
DEPENDENT_LIBS = {
    'openssl': {
        ...
        'sha1'  : '323b175eda887b33fb23f5806ef307b4dda2df00',
        'target': {
            'mingw-w64': {
                'result':   ['include/openssl/ssl.h', 'lib/libssl.a', 'lib/libcrypto.a'],
                'commands': [
                    'perl Configure --prefix=%(dest)s --openssldir=%(dest)s --cross-compile-prefix=%(prefix)s- no-asm no-deprecated mingw',
            ...
            'mingw-w64_x64': {
                'result':   ['include/openssl/ssl.h', 'lib/libssl.a', 'lib/libcrypto.a'],
                'commands': [
                    'perl Configure --prefix=%(dest)s --openssldir=%(dest)s --libdir=lib --cross-compile-prefix=%(prefix)s- no-asm no-deprecated mingw64',
                    'make', 'make install_sw'
                ]
            },
            'msvc': {
                ...
                'commands': [
                    'perl Configure --prefix=%(dest)s --openssldir=%(dest)s no-shared no-asm no-deprecated no-capieng VC-WIN32 -wd4005',
                    'nmake',
                    'nmake install_sw'
                ]
            },
    ...
    'curl': {
        ...
        'sha1'  : '5ff2ecaa4a68ecc06434644ce76d9837e99e7d1d',
        'target': {
            'mingw-w64': {
                'result':   ['include/libssh/libssh.h', 'lib/libcurl.a'],
